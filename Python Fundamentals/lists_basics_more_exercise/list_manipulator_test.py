...
def odd_list(enter: list):
    new_list = [ele for ele in enter if ele % 2 == 1]
    return new_list
...
while True:
    ...
    if current_action == "exchange":
        ...
        if action_list[1] == 'even':
            even_odd_list = even_list(manipulated_list)
            if not even_odd_list:
                print("No matches")
                continue
            max_element = max(even_odd_list)
        elif action_list[1] == 'odd':
            even_odd_list = odd_list(manipulated_list)
            if not even_odd_list:
                print("No matches")
                continue
            max_element = max(even_odd_list)
